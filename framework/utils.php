 ...
 * --------------------------------------------------------------------------- */
function agencies_is_plugin_active($plugin) {
	return in_array( $plugin, (array) get_option( 'active_plugins', array() ) ) || agencies_is_plugin_active_for_network( $plugin );
}

function agencies_is_plugin_active_for_network( $plugin ) {
	if ( !is_multisite() )
		return false;
 ...
 * --------------------------------------------------------------------------- */
function agencies_show_footer_widgetarea( $count ) {
	$classes = array (
		"1" => "dt-sc-full-width",
		"dt-sc-one-half",
		"dt-sc-one-third",
